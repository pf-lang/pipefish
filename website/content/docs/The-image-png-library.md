...
Package `png` supplies a .png encoder and decoder. 

The PNG specification is at https://www.w3.org/TR/PNG/. The `image` library supplies an `Image` type and some basic functions to handle it. The `color` package implements types for representing colors in the usual color models.  
...
### `get(x ref) from (b PngFile)`

Populates the reference variable with an `Image` loaded from the given file.  

### `post(I Image) to (b PngFile)`

Saves the `Image` to the given file as a .png file.  
## Functions 
